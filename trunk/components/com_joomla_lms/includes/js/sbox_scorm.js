		...
		if(reg.test(el.className)){
			...
			if (cur_index > 0 && cur_index <= max_index) {
				this.btnPrev.setStyles({'visibility': 'visible'}).removeEvents("click").addEvent('click', function(e){
					new Event(e).stop();
					jlmsSqueezeBox_scorm.fromElement(all_links[cur_index - 1]);
				});
			} else if(cur_index == 0){
...
jlmsSqueezeBox_scorm.extend(Events.prototype);
jlmsSqueezeBox_scorm.extend(Options.prototype);
jlmsSqueezeBox_scorm.extend(Chain.prototype);
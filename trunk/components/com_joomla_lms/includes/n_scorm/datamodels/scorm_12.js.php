...
function SCORMapi1_2() {
    ...
    var datamodel =  {
        ...
        'cmi.suspend_data':{'defaultvalue':"<?php echo isset($userdata->{'cmi.suspend_data'})?str_replace('"', '\"',str_replace("\r\n", '\r\n', str_replace('\"', '"',$userdata->{'cmi.suspend_data'}))):'' ?>", 'format':CMIString4096, 'mod':'rw', 'writeerror':'405'},
                ...
                if (nav.event != '') {
                    if (nav.event == 'continue') {
                    	if (top.next) {
	                        ...
	                        if (next) {
	                        	setTimeout('top.document.location=next;',500);
	                        }
                       	}
                    } else {
                    	if (top.prev) {
							...
							if (prev) {
	                        	setTimeout('top.document.location=prev;',500);
	                        }
						}
                    }
                } else {
                ...
                if (cmi.core.credit == 'credit') {
                    if (cmi.core.lesson_status == 'completed') {
                        if (cmi.student_data.mastery_score != '' && cmi.core.score.raw != '') {
                        	if (isNaN(parseFloat(cmi.student_data.mastery_score))) {
                        	} else {
...
<?php /* (DEN - 20.09.2007 SefRelToAbs is removed) */
        global $option, $Itemid, $JLMS_CONFIG;
		$user =& JFactory::getUser();
		$user_id = $user->get('id');
		$user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
        $ssid = $user_id.'_'.md5($user_id.'_'.$user_agent);
        $dreq_link = $JLMS_CONFIG->getCfg('live_site')."/index.php?tmpl=component&option=$option&Itemid=$Itemid&task=datamodel_scorm&course_id=$course_id&ssid=$ssid".((isset($skip_resume) && $skip_resume) ? '&skip_resume=1' : '');
?>
        result = DoRequest(myRequest, "<?php echo $dreq_link;?>", "<?php echo "id=$id";?>"+datastring)
